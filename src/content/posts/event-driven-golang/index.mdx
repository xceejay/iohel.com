...
Each step needs to be idempotent and recoverable so that if a request or callback is retried, money is not charged twice and no payments are lost. An event-driven engine gives you a place to store that state, apply retry rules, and recover cleanly after failures.
